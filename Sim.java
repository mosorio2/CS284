...
public class Sim
{
	...
	*/
	public static boolean whenCaught(TSA officer, Passenger pass)
	{
	...
	public static void main(String[] args) throws IOException
	{
		...
		for(int i = 0; i < officerArray.length; i++)
		{
			...
			for(int j = 0; j < sizeLines; j++)
			{
				...
				if((whenCaught(officerArray[i], lines[i].peek())) && (lines[i].peek().getIsThreat() == 1))
					System.out.println(officerArray[i].toString() + " checks on " + lines[i].peek().toString() + "... To the interrogation room. .... Successfull! \n");

				if((whenCaught(officerArray[i], lines[i].peek())) && (lines[i].peek().getIsThreat() == 0))
